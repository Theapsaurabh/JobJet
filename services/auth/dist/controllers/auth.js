...
export const registerUser = tryCatch(async (req, res, next) => {
    ...
    if (role === "recruiter") {
        ...
        const file = req.file;
        const [user] = await sql `INSERT INTO users (name, email, password, phone_number, role, bio, profile_picture, profile_picture_public_id) VALUES (${name}, ${email}, ${hashPassword}, ${phoneNumber}, ${role}, ${bio}, ${file?.path || null}, ${file?.filename || null}) RETURNING user_id, name, email, phone_number, role, bio, profile_picture, created_at`;
    }
    res.json(email);
});
